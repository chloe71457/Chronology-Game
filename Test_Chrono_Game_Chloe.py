...
def _download_to_tempfile(url: str, suffix: str = ".mp3") -> str:
    """Download URL to a temporary file and return the filename (path)."""
...
def play_preview_async(url: str) -> None:
    """
    Download the audio at `url` and play it in a daemon background thread.
    Deletes the temp file after playback completes.
    Non-fatal: prints a message and returns if required libs fail.
    """
    if not url:
        return
    ...
    def _worker(u: str):
        try:
            # lazy import so the rest of the game still runs if playaudio isn't installed
            try:
                from playaudio import playaudio
            except Exception as e:
                print(f"(Preview unavailable — install playaudio: pip install playaudio) [{e}]")
                return

            # Download audio to temp file
            try:
                tmp_path = _download_to_tempfile(u)
            except Exception as ex:
                print(f"(Failed to download preview: {ex})")
                return

            try:
                # playaudio is blocking, so this is OK inside the worker thread
                playaudio(tmp_path)
            except Exception as ex2:
                print(f"(Playback failed: {ex2})")
            finally:
            ...
            # Swallow everything — previews are optional
            return

    t = threading.Thread(target=_worker, args=(url,), daemon=True)
    t.start()
...
def ask_position(timeline: List[Song], challenge: Song) -> Optional[int]:
    ...
    # Start playing preview (non-blocking). If playaudio/requests aren't installed or URL invalid,
    # this will print an explanatory message but won't interrupt gameplay.
    if challenge.track_url:
        play_preview_async(challenge.track_url)
